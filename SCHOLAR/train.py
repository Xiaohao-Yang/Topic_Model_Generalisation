...
parser = argparse.ArgumentParser(description='SCHOLAR')
parser.add_argument("--name", type=str, default="SCHOLAR")
...
def train(model, data_dict, similarity_dict, dissimilarity_dict, rng, bn_anneal=True):
    ...
    for epoch in range(args.epochs):
        ...
        for i in tqdm(range(total_batch), desc='Epoch: %s' % (epoch+1)):
            ...
            cost, recon_y, thetas, nl, kld, reg = model.fit(batch_xs, batch_ys, args.Dist, data_augmented, batch_pcs, batch_tcs)
        ...
        print(meta + "| train loss {:5.2f} (nll {:4.2f} kld {:5.2f} reg loss {:5.2f})"
              .format(avg_cost, avg_nl, avg_kld, avg_reg))
